...
- メモ化再帰するときに、メモリが十分でないときは必要な箇所だけメモすることを考慮しよう。たとえば、 互いに素である数の集合 {a_i} のいずれでも割り切れない N <= 10^13 以下の数を求めたいとき。包除原理で求めることを考えると 2*3*5*…*37*41>10^13 なので、100C12 となり厳しそうだが、a_iを降順にソートして、割った結果 (N/(a_i * a_j * ...))<114514以下の数をメモすれば間に合ってしまう。
