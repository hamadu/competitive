...
public class D {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < q ; i++) {
            ...
            for (int j = 0; j < k ; j++) {
                ...
                mark[special.get(j)] = 1;
            }
            boolean possible = true;
            for (int j = 0 ; j < k ; j++) {
                int par = lca.parent[0][special.get(j)];
                if (par >= 0 && mark[par] == 1) {
                    possible = false;
                    break;
                }
            ...
            if (possible) {
                Collections.sort(special, (o1, o2) -> lca.depth[o2] - lca.depth[o1]);
    ...
    static LCA lca;
    static int[] eulerTable;
    static int[][] eulerPointer;
    static int oid = 0;

    static void dfs0(int now, int par) {
        eulerPointer[now][0] = oid;
        eulerTable[oid++] = now;
        for (int to : lca.graph[now]) {
            if (to == par) {
                continue;
            }
            dfs0(to, now);
        }
        eulerPointer[now][1] = oid;
        eulerTable[oid++] = now;
    }
